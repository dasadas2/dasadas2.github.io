			...
			$products = $this->cart->getProducts();
      		...
      		foreach ($products as $product) {
				...
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                                        $price = $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')), isset($product['currency_code']) ? $product['currency_code'] : '',1);
				} else {
					$price = false;
				}
				...
				// Display prices
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                                        $total = $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity'], isset($product['currency_code']) ? $product['currency_code'] : '',1);
				} else {
					$total = false;
				}
				
        		$this->data['products'][] = array(
					'_mancode'   => $product['_mancode'],
					'_firm'   => $product['_firm'],
					'_country'   => $product['_country'],
					'_articul'   => $product['_articul'],
          			'key'      => $product['key'],
          			'thumb'    => $image,
					'name'     => $product['name'],
          			'model'    => $product['model'],
					...
					'stock1'    => $product['stock1'] ? true : !(!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning')),
