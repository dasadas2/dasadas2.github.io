...
class ControllerPaymentWebmoney extends Controller {
	protected function index() {
		...
		// Переменные
		$this->data['LMI_PAYEE_PURSE'] = $this->config->get('webmoney_merch_r');  // LMI_PAYEE_PURSE Номер кошелька
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/webmoney.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/webmoney.tpl';
		} else {
			$this->template = 'default/template/payment/webmoney.tpl';
		}	
	...
	public function callback() {
		...
		if( ! isset($this->request->post['LMI_PAYEE_PURSE']) ) exit('ERR: Отсутствует номер кошелька продовца');
		
		$LMI_PAYEE_PURSE 		= $this->request->post['LMI_PAYEE_PURSE'];		// Кошелек продовца
		$LMI_PAYMENT_AMOUNT 	= $this->request->post['LMI_PAYMENT_AMOUNT']; 	// Сумма первода 
		...
		$LMI_SECRET_KEY = $this->config->get('webmoney_secret_key');
		...
		if( $order_info['order_status_id'] != $this->config->get('webmoney_order_status_id')) {
			$this->model_checkout_order->update($LMI_PAYMENT_NO, $this->config->get('webmoney_order_status_id'),'Webmoney',TRUE);			
		}
