...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
                ...
                '_firm'            => $query->row['_firm'],
                '_country'         => $query->row['_country'],
                '_mancode'         => $query->row['_mancode'],
                '_articul'         => $query->row['_articul'],
                'currency_code'    => isset($query->row['currency_code']) ? $query->row['currency_code'] : 'RUB',
			);
		} else {
			return false;
		}
	...
	public function getProducts($data = array()) {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "'";
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
			...
			if (!empty($data['filter_name'])) {
				$sql .= " OR LCASE(pd._firm) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
			}

			if (!empty($data['filter_name'])) {
				$sql .= " OR LCASE(pd._country) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
			}

			if (!empty($data['filter_name'])) {
                                $sql .= " OR LCASE(pd._mancode) LIKE '%".$this->db->escape(utf8_strtolower($data['filter_name']))."%'";
			}

			if (!empty($data['filter_name'])) {
				$sql .= " OR LCASE(pd._articul) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
			}
        ...
        public function getRandomProducts($limit) {
                 ...
                 * http://snippetsofcode.wordpress.com/2011/08/01/fast-php-mysql-random-rows/
                  ...
                  $sql="SELECT id FROM pages WHERE RAND()*$num_records<$limit*2
                ...
                //$sql .= " AND p.quantity > 0 ";
                // where place
                $sql .= " AND RAND()*$num_records<$limit*2 ";
                $sql .= " GROUP BY p.product_id";
	...
	public function getProductSpecials($data = array()) {
		...
		$sql = "SELECT DISTINCT ps.product_id, (SELECT AVG(rating) FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = ps.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < '" . $this->NOW . "') AND (ps.date_end = '0000-00-00' OR ps.date_end > '" . $this->NOW . "')) GROUP BY ps.product_id";
	...
	public function getLatestProducts($limit) {
		...
		if (!$product_data) {
                        $queryNulls = '';
                        if ($this->hideNulls) {
                                $queryNulls = ' AND p.quantity > 0 ';
                        }
                        $query = $this->db->query("SELECT p.product_id FROM ".DB_PREFIX."product p WHERE p.status = '1' AND p.date_available <= '" . $this->NOW . "' " . $queryNulls . " ORDER BY p.date_added DESC LIMIT ".(int) $limit);
	...
	public function getPopularProducts($limit) {
		$product_data = array();
                ...
                $query = $this->db->query("SELECT p.product_id FROM ".DB_PREFIX."product p WHERE p.status = '1' AND p.date_available <= '" . $this->NOW . "' " . $queryNulls . " ORDER BY p.viewed, p.date_added DESC LIMIT ".(int) $limit);
	...
	public function getBestSellerProducts($limit) {
		...
		if (!$product_data) {
			...
			$query = $this->db->query("SELECT op.product_id, COUNT(*) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (op.product_id = p.product_id) WHERE o.order_status_id > '0' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "' GROUP BY op.product_id ORDER BY total DESC LIMIT " . (int)$limit);
	...
	public function getProductRelated($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_related pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_id = p.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= '" . $this->NOW . "'");
	...
	public function getTotalProductSpecials() {
		...
		$query = $this->db->query("SELECT COUNT(DISTINCT ps.product_id) AS total FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) WHERE p.status = '1' AND p.date_available <= '" . $this->NOW . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < '" . $this->NOW . "') AND (ps.date_end = '0000-00-00' OR ps.date_end > '" . $this->NOW . "'))");
	...
	public function getProductsByModel($model) {		
		$query = $this->db->query("select *, ROUND(p.price,2) AS price, pd.name as product_name, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < '" . $this->NOW . "') AND (ps.date_end = '0000-00-00' OR ps.date_end > '" . $this->NOW . "')) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special from " . DB_PREFIX . "product p left join " . DB_PREFIX . "product_description pd on (p.product_id=pd.product_id and pd.language_id='" . (int)$this->config->get('config_language_id') . "') where p.model='".$this->db->escape($model)."' and p.status<>0");
		return $query->rows; 	
	}
	
	public function AcatSearchByModel($model) {
		$query = $this->db->query("select * from ".AC_DB_PREFIX."search where number='".$model."';");
		return $query->rows;
	}
}
?>