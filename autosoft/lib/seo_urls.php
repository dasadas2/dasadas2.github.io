...
//    require_once('config.php');
    ...
    //echo 'generateSEO<br />';
    //include_once('/home/g/gplus/im/public_html/admin/config.php');
    ...
    $query_product = $db->query("SELECT p.product_id, p.model, pd.name as name FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)");
    ...
    foreach ($query_product->rows as $result) {
        ...
        if (!empty($result['model'])) {
            $keyword .= "-" . title2uri($result['model']);
        }
