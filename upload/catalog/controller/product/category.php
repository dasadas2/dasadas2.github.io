		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				if ($this->config->get('config_tax')) {
					$tax = $this->currency->format((float)$result['special'] ? $result['special'] : $result['price'],isset($result['currency_code'])?$result['currency_code']:'',1);
				} else {
					$tax = false;
				}
				...
				$this->data['products'][] = array(
					...
					'name'        => $result['name'],
                                                                      'model' =>$result['model'],
					'_mancode'   => $result['_mancode'],
					'_firm'   => $result['_firm'],
					'_country'   => $result['_country'],
					'_articul'   => $result['_articul'],
					'quantity'   => $result['quantity'],
					'stock'   => $result['stock'],
					'description' => utf8_substr(strip_tags(str_replace('<br>',' ',html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0, 100) . (utf8_strlen(strip_tags(str_replace('<br>',' ',html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))))>100 ? '..' : ''),
					'price'       => $price,
					'special'     => $special,
					'tax'         => $tax,
