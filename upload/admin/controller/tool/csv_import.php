	...
	public function index() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate()) {
			...
			if ($filename) {
				...
				if ($this->request->post['csv_import_type'] == 'fullreset') {
					$this->model_tool_csv_import->emptyDataBase();
					$this->session->data['success']="База данных очищена.";
					$this->redirect($this->url->link('tool/csv_import', 'token=' . $this->session->data['token'], 'SSL'));
				} else {
	    ...
	    while (($raw_prod = $this->getNextProduct($file)) !== FALSE)
	    {
			...
			foreach (array('price', 'cost') as $price) {
				if (isset($this->field_names[$price]) && isset($raw_prod[$this->field_names[$price]])) {
					...
					$raw_prod[$this->field_names[$price]] = str_replace(',', '.', $raw_prod[$this->field_names[$price]]); //igor было  str_replace(',', '', $raw_prod[$this->field_names[$price]]);
				}
			}
...
//igor echo sprintf('<script type="text/javascript">var win=window.open("","popup","width=200,height=200,left=300,top=400,menubar=0,resizable=0,scrollbars=0,titlebar=0,toolbar=0",true); win.document.writeln("%s");<///script>',$raw_prod['price']);
            ...
            foreach($this->discounts as $discount) {
                $price_in = str_replace(',', '.', $raw_prod['price']); //igor было =$raw_prod['price'];
	...
	private function fetchFeed() {
		if (is_uploaded_file($this->request->files['csv_import']['tmp_name'])) {
            ...
            $this->import_filename = $this->request->files['csv_import']['name'];
		}
