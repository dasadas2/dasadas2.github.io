...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
			...
			if (!empty($data['filter_name'])) {
                                //$sql .= " OR LCASE(pd._mancode) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
	...
	public function getLatestProducts($limit) {
		...
		if (!$product_data) {
                        ...
                        $query = $this->db->query("SELECT p.product_id FROM ".DB_PREFIX."product p LEFT JOIN ".DB_PREFIX."product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '".(int) $this->config->get('config_store_id')."'".$queryNulls." ORDER BY p.date_added DESC LIMIT ".(int) $limit);
	...
	public function getPopularProducts($limit) {
		$product_data = array();
                ...
                $query = $this->db->query("SELECT p.product_id FROM ".DB_PREFIX."product p LEFT JOIN ".DB_PREFIX."product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '".(int) $this->config->get('config_store_id')."'".$queryNulls." ORDER BY p.viewed, p.date_added DESC LIMIT ".(int) $limit);
